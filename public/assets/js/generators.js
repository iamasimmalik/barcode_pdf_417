...
function makeDOEfromDOI(years) {
    ...
    if (!dateOfIssue || dateOfIssue.length != 8) {
        return;
    }
...
function makeRandomDOE_plus_day(diff) {
    ...
    var year = birthDateDATE.toLocaleString("default", {
        year: "2-digit"
    });
    var month = birthDateDATE.toLocaleString("default", {
        month: "2-digit"
    });
    var day = birthDateDATE.toLocaleString("default", {
        day: "2-digit"
    });
...
function makeRandomDOE_minus_day(diff) {
    ...
    // Get year, month, and day part from the date
    var year = expiryDateDATE.toLocaleString("default", {
        year: "2-digit"
    });
    var month = expiryDateDATE.toLocaleString("default", {
        month: "2-digit"
    });
    var day = expiryDateDATE.toLocaleString("default", {
        day: "2-digit"
    });
...
function MS_calculate_ICN() {
    document.getElementById("inputICN").value = "05100" + getRandomNumericString(6) + "23" + getRandomNumericString(3);
}
...
function MS_calculate_DD() {
    ...
    d = ("00" + (parseInt(getNumberOfDaysFromBeginnigOfYear(issueDate)))).slice(-3);
...
function TX_2016_calculate_ICN() {
    ...
    // Get year, month, and day part from the date
    var year = issueDateDATE.toLocaleString("default", {
        year: "numeric"
    });
    var month = issueDateDATE.toLocaleString("default", {
        month: "2-digit"
    });
    var day = issueDateDATE.toLocaleString("default", {
        day: "2-digit"
    });
...
function ND_2023_calculate_ICN() {
    document.getElementById("inputICN").value = "05100" + getRandomNumericString(11);
}
...
function ND_calculate_DD() {
    ...
    if (!height) {
        showInputDataAlert("DD calculation error. Incorrect height!")
    }
    ...
    var sex = document.getElementById("inputSex").value
    if (!sex) {
        showInputDataAlert("DD calculation error. Incorrect Sex!")
    }

    sex_digit = sex == "M" ? "5" : "1"

    document.getElementById('inputDD').value = birthDate.slice(-2, -1) + documentNumber + lastName[0] + firstName[0] +
        sex_digit + "2" + expiryDate.slice(-1) + expiryDate.slice(2, 4) + getRandomLetter().toLowerCase() + getRandomLetter() + height + "NDZ";
}

function ND_2023_calculate_DD() {
    var birthDate = document.getElementById("inputBrithDate").value;
    if (birthDate.length != 8) {
        showInputDataAlert("DD calculation error. Incorrect birth date!")
        return;
    ...
    document.getElementById('inputDD').value = birthDate.slice(-2, -1) + documentNumber + lastName[0] + firstName[0] +
        sex_digit + "2" + expiryDate.slice(-1) + expiryDate.slice(2, 4) + getRandomLetter().toLowerCase() + getRandomLetter() + height + "YDZ";
}
...
function MI_calculate_ICN() {
    ...
    document.getElementById("inputICN").value = documentNumber.replace(/\s/g, '') +
        birthDate.slice(-4) + birthDate.slice(0, 2) + birthDate.slice(2, 4) + expiryDate.slice(-2) + expiryDate.slice(0, 2);
}
...
function NJ_2020_calculate_ICN() {
    ...
    var random_data = "";
    for (var i = 0; i < 9; i++) {
        random_data += getRandomLetterAndDigit();
    }
    random_data += "NJ"
...
function YT_calculate_documentNumber() {
    document.getElementById('inputDocumentNumber').value = "1" + getRandomNumericString(5);
}

function YT_calculate_DD() {
    var documentNumber = document.getElementById("inputDocumentNumber").value;
    ...
    document.getElementById('inputDD').value = documentNumber + "R0"
}
